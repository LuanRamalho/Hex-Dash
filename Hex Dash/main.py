...
# Carregar o High Score inicial
high = load_high_score(HIGH_SCORE_FILE)
...
while running:
    ...
    if score_page:
        ...
        bestscore_msg.update(high, shadow=False)  # Mostrar o High Score
    ...
    if game_page:
        ...
        if player.rect.collidepoint(bar.get_center()):
            ...
            if score > high:
                high = score
                save_high_score(HIGH_SCORE_FILE, high)  # Salvar novo High Score
            if score % 3 == 0:
                cindex = (cindex + 1) % 5
                ccolor = COLORS[cindex]
